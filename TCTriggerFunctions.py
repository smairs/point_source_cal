...
def TCCoadd(input_data):                                                                                                                                                     
    ...
    output_name    = sorted(input_data)[-1].split('.sdf')[0]+'_coadd'
...
def TCMetadata(input_data,region,output_dir,wave='850'):
    '''                    
    ...
    if len(existing_metadata_file)>0:
        ...
        if t['Name'][-1] == region+'_'+np.array(input_data)[np.argsort(np.array(input_data))][-1].split('_')[-6]+'_'+np.array(input_data)[np.argsort(np.array(input_data))][-1].split('_')[-5]+'_'+wave:
            anythingnew = 0
            input_data = []
	# If the table already exists and we are adding something new, just get information from the most current file and append it to the existing table
        if anythingnew == 1:
    ...
    for eachfile in input_data:
        ...
        metadata   = np.loadtxt(picard_out.logfiles[0],dtype={'names':('UT','HST','Obs','Source','Mode','Filter',
        ...
        noises.append(str(metadata['rms']))
    ...
    if len(input_data)>0:
        ...
        apascii.write(t,region+'_'+mostrecentdate+'_'+mostrecentscan+'_'+wave+'_metadata.txt')

        os.system('mv -f '+region+'_'+mostrecentdate+'_'+mostrecentscan+'_'+wave+'_metadata.txt '+output_dir)
...
def TCCheck4Variables(source_dict,YSOtable,region,trigger_thresh = 5,brightness_thresh = 0.0,sd_thresh = 4, wave ='850'): 
    '''
    ...
    for eachsource in source_dict.keys():                            
        ...
        source_dict[eachsource]['sd_fiducial'] = np.sqrt(0.014**2.0+(0.02*np.average(source_dict[eachsource]['peakfluxes']))**2.0)
        ...
        source_dict[eachsource]['slope']           = slope*365.24 # Now in %/year
        source_dict[eachsource]['delta_slope']     = delta_slope*365.24
...
##########
        ...
        for eachprop in [source_dict[eachsource]['mean'],source_dict[eachsource]['sd'],source_dict[eachsource]['sd_fiducial'],source_dict[eachsource]['sd_fiducial_trigger'],slope,delta_slope,abs(slope/delta_slope),intercept,delta_intercept]:
    ...
    #mostrecentdate = np.array(input_data)[np.argsort(np.array(input_data))][-1].split('/')[-1].split('_')[1]
    #mostrecentscan = np.array(input_data)[np.argsort(np.array(input_data))][-1].split('/')[-1].split('_')[2]
    apascii.write(t,region+'_'+mostrecentdate+'_'+mostrecentscan+'_'+wave+'_sourceinfo.txt')
