def TCTrigger(input_data,protocat,diskcat,region, aperture_diam = 0.00083333, trigger_thresh = 5, brightness_thresh = 0.0, sd_thresh = 2,wave='850',EXTRASOURCES=False,ONLYEXTRA=False):
    '''
    ...
    from TCTriggerFunctions import TCCoadd,TCMetadata,TCIdentifySources,TCYSOcompare,TCTrackSources,TCCheck4Variables
    import os
    import numpy as np
...
#
	...
	###### Build Metadata Table ######
        ...
        metadata = TCMetadata(input_data,region,output_dir+'/'+region,wave=wave)
...
#
	...
	######### Load Source Catalogue ##########
        ...
        # So use the catalogues Doug created for his midproject paper

        #peakcat_name = TCIdentifySources(output_dir+coadd_name.split('/')[-1])
    
        peakcat_name = '/export/data/smairs/smairs_github/450_cal_check_20190413/config/'+region+'_450_sourcecat_20201201.fits'
...
#
	    ...
	    ### Perform the Tests for Variability ###
            ...
            source_slope_table,triggered_sources = TCCheck4Variables(source_dict,YSOtable,region,trigger_thresh = trigger_thresh,brightness_thresh = brightness_thresh,sd_thresh = sd_thresh,wave=wave)
	    ...
	    #### Add pound sign to each txt file if it doesn't have it already ####
            ...
            needtooverwrite= 0
            f    = open(output_dir+'/'+region+'/'+region+'_'+mostrecentdate+'_'+mostrecentscan+'_'+wave+'_sourceinfo.txt')
            ...
            if needtooverwrite==1:
                os.system('mv -f newfile.txt '+output_dir+'/'+region+'/'+region+'_'+mostrecentdate+'_'+mostrecentscan+'_'+wave+'_sourceinfo.txt')
	    ...
	    # YSOcompare File
            ...
            # Metadata file
            needtooverwrite = 0
            f    = open(output_dir+'/'+region+'/'+region+'_'+mostrecentdate+'_'+mostrecentscan+'_'+wave+'_metadata.txt')
            newf = open('newfile.txt','w')
            lines = f.readlines() # read old content
            if lines[0][0]=='#':
                newf.close()
            ...
            if needtooverwrite == 1:
                os.system('mv -f newfile.txt '+output_dir+'/'+region+'/'+region+'_'+mostrecentdate+'_'+mostrecentscan+'_'+wave+'_metadata.txt')
            #########################################
	    #########################################
...
#
    ...
    if EXTRASOURCES == True:
        from TCExtraSourceAnalysis_beta import TCYSOcompareES,TCTrackSourcesES,TCCheck4VariablesES
